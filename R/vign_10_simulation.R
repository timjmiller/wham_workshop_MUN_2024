...
saveRDS(fit_1, file.path("temp", "vign_10_fit_1.RDS"))
...
saveRDS(simfit, file.path("temp", "vign_10_simfit_1.RDS"))
...
saveRDS(simfit_2, file.path("temp", "vign_10_simfit_2.RDS"))
...
saveRDS(stock_om, file.path("temp", "vign_10_stock_om_1.RDS"))
...
saveRDS(simfit_3, file.path("temp", "vign_10_simfit_3.RDS"))
...
get_F_from_catch <- function(om, year, catch, Finit = 0.1, maxF = 10){
    ...
    sel_tot = rbind(rep$FAA[,year,]/exp(rep$log_F_tot[year])) #matrix nfleets x n_ages 
...
saveRDS(stock_om_fb, file.path("temp", "vign_10_stock_om_fb.RDS"))
...
saveRDS(looped_res, file.path("temp", "vign_10_looped_res.RDS"))
