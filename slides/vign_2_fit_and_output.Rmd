...
* `report`: A function of `par` that generates a list of derived variables the C++ code is specified to provide.
* `simulate`: A function of `par` that will simulate any variables and observations the C++ code is specified to simulate given the value of `par`
...
* `rep`: the list of reported variables returned by `$report()`. Analogous to the `*.rep` files produced using ADMB.
...
```
The most minimal use of fit_wham (with `do.fit = TRUE`) turns off all post-fitting features:
```{r, eval = FALSE}
...
It is easy to add standard error estimation to a model that has already been fit:
```{r, include = FALSE}
...
`log_NAA` is an array (n_stocks x n_regions x n_years-1 x n_ages) of parameters for log abundance at age parameters.
 * annual recruitment could be fixed or random effects
 * numbers at older ages can be random effects (or not used)
...
```
It is also easy to add reference reporting to a model that has already been fit:
```{r, eval = FALSE}
fit_asap <- do_reference_points(fit_asap, do.sdrep = TRUE)
```

and there are also functions for adding retro peels and one-step-ahead residuals
```{r, eval = FALSE}
fit_asap$peels <- retro(fit_asap)
fit_asap <- make_osa_residuals(fit_asap)
```
...
WHAM allows alternative output types when creating plots and tables using `plot_wham_output`.
The default call is
```{r, eval = FALSE}
...
`out.type = "html"` will use Rmarkdown and pandoc to create a self-contained html file of both plots and tables that can be moved, shared, and viewed at any time in a browser.
...
knitr::include_graphics(here::here("slides", "figs", "output_img_1.png"))
```
---
...
knitr::include_graphics(here::here("slides", "figs", "output_img_2.png"))
```
]
.pull-right[
```{r, echo = FALSE, out.width="100%", fig.align="center"}
knitr::include_graphics(here::here("slides", "figs", "output_img_3.png"))
```
]
---
# Plotting output
.pull-left[
```{r, echo = FALSE, out.width="100%", fig.align="center"}
knitr::include_graphics(here::here("slides", "figs", "output_img_4.png"))
```
]
.pull-right[
```{r, echo = FALSE, out.width="100%", fig.align="center"}
knitr::include_graphics(here::here("slides", "figs", "output_img_5.png"))
```
]
...
.pull-left[
```{r, echo = FALSE, out.width="100%", fig.align="center"}
knitr::include_graphics(here::here("slides", "figs", "output_img_6.png"))
```
]
.pull-right[
```{r, echo = FALSE, out.width="100%", fig.align="center"}
knitr::include_graphics(here::here("slides", "figs", "output_img_7.png"))
```
]
---
...
.pull-left[
```{r, echo = FALSE, out.width="100%", fig.align="center"}
knitr::include_graphics(here::here("slides", "figs", "output_img_9.png"))
```
]
.pull-right[
```{r, echo = FALSE, out.width="100%", fig.align="center"}
knitr::include_graphics(here::here("slides", "figs", "output_img_10.png"))
```
]
